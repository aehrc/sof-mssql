...
import { FHIRPathTranspiler, TranspilerContext } from '../src/fhirpath-transpiler';
...
describe('FHIRPathTranspiler', () => {
  ...
  describe('property navigation', () => {
    it('should transpile nested property access', () => {
      const result = FHIRPathTranspiler.transpile('name.family', defaultContext);
  ...
  describe('functions', () => {
    it('should transpile exists() function', () => {
      const result = FHIRPathTranspiler.transpile('id.exists()', defaultContext);
      expect(result).toContain('IS NOT NULL');
    });

    it('should transpile getResourceKey() function', () => {
      const result = FHIRPathTranspiler.transpile('getResourceKey()', defaultContext);
      expect(result).toBe('r.id');
    });

    it('should transpile first() function', () => {
      const result = FHIRPathTranspiler.transpile('name.first()', defaultContext);
      expect(result).toContain('JSON_VALUE');
  ...
  describe('operators', () => {
    it('should transpile equality operators', () => {
      const result = FHIRPathTranspiler.transpile("gender = 'male'", defaultContext);
      expect(result).toContain('=');
      expect(result).toContain("'male'");
    });

    it('should transpile logical operators', () => {
      const result = FHIRPathTranspiler.transpile('active and gender.exists()', defaultContext);
  ...
  describe('error handling', () => {
    it('should throw error for unsupported functions', () => {
      expect(() => FHIRPathTranspiler.transpile('unsupportedFunction()', defaultContext))
        .toThrow('Unsupported FHIRPath function');
    });

    it('should throw error for invalid expressions', () => {
      expect(() => FHIRPathTranspiler.transpile('invalid syntax...', defaultContext))
